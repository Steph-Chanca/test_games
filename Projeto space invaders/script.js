...
//################### Tank e sprites gerais
var frameCount = 0;
var armyPrevFrameCount = 0;
var framesInOneSec = 1000/16;
var spriteUnitHeight = 35;
var spriteUnitWidth = 64;
var scoreBarHeight = 50;
var tank__bottomOffset = (spriteUnitHeight / 2) + scoreBarHeight;
...
//############################### Handlers / localização exata de onde os projéteis serão gerados
...
function moveInvaderBullets() {
    for (let i = 0; i < invaderBulletsArray.length; i++) {
        ...
        // Verifica se a bala está fora dos limites de tela
        if (iB.y > canvas.height) {
            invaderBulletsArray.splice(i,1);
        }
        // Verifica se houve game over por hits da bala no defensor
        if(
            iB.x > tankX &&
...
function helpHandler() {
    if (aliveInvaders == armyColumns * armyRows) {
        drawBottomMessage("aperte ESPAÇO para atirar", 125);
    }
    else if (hasLifeDecreased) {
        drawBottomMessage(`HIT. Vidas restantes: ${lives}`, 150);
        setTimeout(() => {
            hasLifeDecreased = false;
...
function drawTank(x,y) {
  ...
  function drawBullet(bx,by) {
      ctx.beginPath();
      ctx.beginPath(); // Começando um novo path
      ctx.moveTo(bx,by); // Move a caneta para (30, 50)
      ctx.lineTo(bx, by - bullet__height); // Cria uma linha de segmento
  ...
  function moveTankBullet() {
      ...
      for (let i = 0; i < armyRows; i++) {
          ...
          )
          {
                    ...
                    // Incremento de velocidade dos invasores
  ...
  // Verifica se o invasor foi atingido
  tankBullet__y -= tankBullet__dy;
}

function drawScoreSeprateLine() {
    ctx.beginPath();
    ctx.beginPath(); // Começa um novo path
...
function drawBottomHelper() {
    drawBottomMessage("aperte enter para recomeçar", 115)
}

function drawInvaderBullet(ix,iy) {
    ctx.beginPath();
    ctx.beginPath();
    ctx.moveTo(ix,iy); // Move a caneta de desenho
    ctx.lineTo(ix,iy + bullet__height); // Desenha a linha
...
function drawStartScreen() {
    drawBlinker(function(){ drawScreen__line1("Space Invaders")},function(){ drawScreen__line2("Aperte enter para jogar")});
}
...
function drawExplosion() {
    ...
    for(let i = 0; i < explosions.length; i++) {
        ...
        for(let ii = 0; ii < particles.length; ii++) {
            ...
            // Verifica se a partícula tem tamanho 0, se sim, a remove
            if(particle.size <= 0) {
